...
export class PaymentType implements OnInit{

  public apiUrl = ApiRoutesConstants.BASE_URL+ ApiRoutesConstants.payment_type_getlist;
  ...
  constructor(private navService: Data, private router: Router,private route :ActivatedRoute,
    ...
    this.columnDefinition = HeaderConstants.paymentTypeListHeader;
  }
  ngOnInit(): void {
    this.getpaytype();
  }
    ...
    this.navService.getData(this.apiUrl).subscribe({
      next:(res:any)=> {
        ...
        if (res.Code === 200) {
          this.ListData = res.Data.payment_types;
          console.log('roledate',this.ListData);
    ...
    if (event.actions === 'Edit') {
      this.router.navigate(['/admin/payment-type-list/edit',event.data._id]);
      ...
      dialogRef.afterClosed().subscribe((confirmed: boolean) => {
        if (confirmed) {
          let apiUrl = ApiRoutesConstants.BASE_URL+ ApiRoutesConstants.PAYMENT_TYPE + ApiRoutesConstants.DELETE;
