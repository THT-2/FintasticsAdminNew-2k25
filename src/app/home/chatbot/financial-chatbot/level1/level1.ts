...
export class Level1 {
  ...
  buttondata= {
    buttonName : 'Add New Level 1',
    routingPath : '/admin/level1/create',
    // routingView : 'View',
    routingEdit : 'Edit',
  ...
  getActions(event:any){
    ...
    if (event.actions === 'Edit') {
      this.router.navigate(['/admin/level1/edit',event.data._id]);
      ...
      dialogRef.afterClosed().subscribe((confirmed: boolean) => {
        if (confirmed) {
          ...
          this.navService.postData(apiUrl,{tableName:"level1Schemachat"}).subscribe({
            next: (res: any) => {
