...
export class Level3 {
  ...
  buttondata= {
    buttonName : 'Add New Level 3',
    routingPath : '/admin/level3/create',
    routingView : 'View',
    routingEdit : 'Edit',
  ...
  getActions(event:any){
    ...
    if (event.actions === 'Edit') {
      this.router.navigate(['/admin/level3/edit',event.data._id]);
      ...
      dialogRef.afterClosed().subscribe((confirmed: boolean) => {
        if (confirmed) {
          ...
          this.navService.postData(apiUrl,{tableName:"Level3Schemachat"}).subscribe({
            next: (res: any) => {
