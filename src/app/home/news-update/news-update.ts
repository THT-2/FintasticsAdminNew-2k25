...
export class NewsUpdate implements OnInit{


  public apiUrl = ApiRoutesConstants.BASE_URL+ ApiRoutesConstants.newscroll_getlist;
  ...
  constructor(private navService: Data, private router: Router,private route :ActivatedRoute, private cdr: ChangeDetectorRef,private alertService: AlertService, private dialog: MatDialog){
    this.columnDefinition = HeaderConstants.NewsHeader;
  }
  ngOnInit(): void {
    this.getupdates();
  }
    ...
    if (event.actions === 'Edit') {
      this.router.navigate(['/admin/news-update/edit',event.data._id]);
